...
def run_setup():
    ...
    for integration in manager.get_supported_integrations():
        ...
        enabled = prompt_integration_enabled(
            integration
        )
        ...
        if not integration.validate():
            ...
            print(
                f"{integration.name} validation failed."
            )
...
def prompt_integration_enabled(integration):
    ...
    if integration.required:
        ...
        print(
            f"{integration.name} is required."
        )

        return True

    response = input(
        f"Enable {integration.name}? (y/n): "
    )
